import{x as ge,m as ve,l as je,r,s as $,j as e,n as ye,d as g,a3 as Ne,P as ke,C as m,t as q,v as E,ad as V,B as x,w as p,_ as we,al as K,N as W,a1 as Ce,K as Je,S as X,$ as Y,z as R,I as _e,X as Se,D as Z,E as ee,F as se,G as te,J as v,aQ as Me,a0 as ae}from"./index-1760451543780.js";import{C as F}from"./check-circle-2-1760451543780.js";import{T as De}from"./trending-up-1760451543780.js";const Pe=ge("Printer",[["polyline",{points:"6 9 6 2 18 2 18 9",key:"1306q4"}],["path",{d:"M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2",key:"143wyd"}],["rect",{width:"12",height:"8",x:"6",y:"14",key:"5ipwut"}]]),Ae=()=>{const{isAdmin:I,isAdherent:le}=ve(),{toast:k}=je(),[A,ie]=r.useState([]),[c,re]=r.useState([]),[b,ne]=r.useState(null),[T,B]=r.useState([]),[d,w]=r.useState(null),[oe,j]=r.useState(!0),[C,G]=r.useState(""),[_,de]=r.useState("all"),[S,ce]=r.useState("all");r.useState("cards");const[o,M]=r.useState(!1),[J,y]=r.useState(null);r.useEffect(()=>{O()},[]);const O=async()=>{j(!0);try{const{data:s,error:t}=await $.from("members").select("*").not("passeport","is",null).order("last_name").order("first_name");if(t)throw t;const{data:l,error:a}=await $.from("passeport_validations").select("*").order("date_validation",{ascending:!1});if(a)throw a;ie(s||[]),re(l||[])}catch{k({title:"Erreur",description:"Impossible de charger les données",variant:"destructive"})}finally{j(!1)}},H=async s=>{j(!0);try{const{data:t,error:l}=await $.from("passeport_validations").select("*").eq("member_id",s).order("date_validation",{ascending:!1});if(l)throw l;B(t||[]),t&&t.length>0&&w(t[0])}catch{k({title:"Erreur",description:"Impossible de charger les validations",variant:"destructive"})}finally{j(!1)}},U=s=>{const t=A.find(l=>l.id===s);ne(t),B([]),w(null),t&&H(s)},ue=()=>{y({...d}),M(!0)},me=()=>{y(null),M(!1)},D=s=>{y(t=>({...t,competences:{...t.competences,[s]:!t.competences[s]}}))},pe=s=>{y(t=>({...t,observations:s}))},xe=async()=>{try{j(!0);const{error:s}=await $.from("passeport_validations").update({competences:J.competences,observations:J.observations}).eq("id",J.id);if(s)throw s;k({title:"Succès",description:"Le passeport a été mis à jour avec succès."}),await H(b.id),await O(),w(J),M(!1),y(null)}catch(s){k({title:"Erreur",description:"Impossible de mettre à jour le passeport.",variant:"destructive"}),console.error("Erreur de mise à jour:",s)}finally{j(!1)}},he=()=>{const s=window.open("","_blank");if(!s){k({title:"Erreur",description:"Impossible d'ouvrir la fenêtre d'impression. Vérifiez que les popups ne sont pas bloquées.",variant:"destructive"});return}const t=d,l=b,a=t.passeport_type.charAt(0).toUpperCase()+t.passeport_type.slice(1),n=t.module?` - Module ${t.module==="bloc"?"Bloc":"Difficulté"}`:"",h=Object.entries(t.competences||{}),i=h.filter(([$e,fe])=>fe===!0).length,u=h.length,f=`<svg width="120" height="120" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
