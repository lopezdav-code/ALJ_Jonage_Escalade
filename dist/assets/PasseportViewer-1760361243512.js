import{c as ge,Q as ve,O as je,r,t as P,j as e,R as ye,B as g,a1 as Ne,z as ke,C as m,b as $,d as V,a8 as E,x,e as p,Y as we,aj as Y,g as Z,aa as Ce,T as Je,b4 as _e,S as W,Z as X,L as R,I as Se,X as Me,n as K,o as ee,p as se,q as te,s as v,aU as qe,_ as ae}from"./index-1760361243512.js";import{C as F}from"./check-circle-2-1760361243512.js";const De=ge("Printer",[["polyline",{points:"6 9 6 2 18 2 18 9",key:"1306q4"}],["path",{d:"M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2",key:"143wyd"}],["rect",{width:"12",height:"8",x:"6",y:"14",key:"5ipwut"}]]),Ee=()=>{const{isAdmin:I,isAdherent:le}=ve(),{toast:k}=je(),[A,ie]=r.useState([]),[d,re]=r.useState([]),[b,ne]=r.useState(null),[T,B]=r.useState([]),[c,w]=r.useState(null),[oe,j]=r.useState(!0),[C,G]=r.useState(""),[_,ce]=r.useState("all"),[S,de]=r.useState("all");r.useState("cards");const[o,M]=r.useState(!1),[J,y]=r.useState(null);r.useEffect(()=>{O()},[]);const O=async()=>{j(!0);try{const{data:s,error:t}=await P.from("members").select("*").not("passeport","is",null).order("last_name").order("first_name");if(t)throw t;const{data:l,error:a}=await P.from("passeport_validations").select("*").order("date_validation",{ascending:!1});if(a)throw a;ie(s||[]),re(l||[])}catch{k({title:"Erreur",description:"Impossible de charger les données",variant:"destructive"})}finally{j(!1)}},U=async s=>{j(!0);try{const{data:t,error:l}=await P.from("passeport_validations").select("*").eq("member_id",s).order("date_validation",{ascending:!1});if(l)throw l;B(t||[]),t&&t.length>0&&w(t[0])}catch{k({title:"Erreur",description:"Impossible de charger les validations",variant:"destructive"})}finally{j(!1)}},H=s=>{const t=A.find(l=>l.id===s);ne(t),B([]),w(null),t&&U(s)},ue=()=>{y({...c}),M(!0)},me=()=>{y(null),M(!1)},q=s=>{y(t=>({...t,competences:{...t.competences,[s]:!t.competences[s]}}))},pe=s=>{y(t=>({...t,observations:s}))},xe=async()=>{try{j(!0);const{error:s}=await P.from("passeport_validations").update({competences:J.competences,observations:J.observations}).eq("id",J.id);if(s)throw s;k({title:"Succès",description:"Le passeport a été mis à jour avec succès."}),await U(b.id),await O(),w(J),M(!1),y(null)}catch(s){k({title:"Erreur",description:"Impossible de mettre à jour le passeport.",variant:"destructive"}),console.error("Erreur de mise à jour:",s)}finally{j(!1)}},he=()=>{const s=window.open("","_blank");if(!s){k({title:"Erreur",description:"Impossible d'ouvrir la fenêtre d'impression. Vérifiez que les popups ne sont pas bloquées.",variant:"destructive"});return}const t=c,l=b,a=t.passeport_type.charAt(0).toUpperCase()+t.passeport_type.slice(1),n=t.module?` - Module ${t.module==="bloc"?"Bloc":"Difficulté"}`:"",h=Object.entries(t.competences||{}),i=h.filter(([Pe,fe])=>fe===!0).length,u=h.length,f=`<svg width="120" height="120" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
